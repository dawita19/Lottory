            ...
            for _ in range(rewards_to_award):
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=f"🎉 Congratulations! You've bought 10 tickets and received a FREE 100 Birr ticket: #{chosen_number}! Check /mytickets to see your new ticket!"
                    )
                except TelegramError as e:
...
# This function initializes the database and starts the background schedulers.
# It is called by the bot and doesn't need to be run separately.
def initialize_common_components():
    global _initialization_done
    if _initialization_done:
        return

    logger.info("Performing common application initialization (DB & schedulers)...")
    init_db()
    LotteryBot.init_schedulers_standalone()
    _initialization_done = True
    logger.info("Common application components initialized.")

# Call common initialization when this module is imported.
initialize_common_components()

# --- Main entry point for the bot.py worker service ---
if __name__ == "__main__":
    logger.info("Starting bot.py as the main worker process.")
    try:
        # Initialize and run the bot application directly.
        # This uses the main asyncio event loop for polling.
        bot = LotteryBot()
        asyncio.run(bot.start_polling())
    except KeyboardInterrupt:
        logger.info("Bot worker stopped by KeyboardInterrupt.")
    except Exception as e:
        logger.critical(f"Unhandled exception in bot worker startup: {e}", exc_info=True)
