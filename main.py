...
from telegram.ext import (
    ...
    filters,
    TypeHandler,
    CallbackQueryHandler # Ensure CallbackQueryHandler is imported
)
...
class LotteryBot:
    ...
    async def _cancel_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        user_id = update.effective_user.id # Telegram user ID from update
        tier = context.user_data.get('tier')
        number = context.user_data.get('number')
        ...
        if tier and number:
            with Session() as session:
                try:
                    # Fetch the User object from the database using telegram_id
                    user_db = session.query(User).filter_by(telegram_id=user_id).first()
                    if user_db:
                        # Remove the reservation using the user's database ID
                        session.query(ReservedNumber).filter_by(
                            user_id=user_db.id, # Fixed: Use user_db.id here
                            tier=tier,
                            number=number
                        ).delete()
                        session.commit()
                        logging.info(f"Reservation for user {user_db.id}, number {number}, tier {tier} cancelled.")
                except SQLAlchemyError as e:
                    session.rollback()
